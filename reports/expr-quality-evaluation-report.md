...
2. ~~`eval/parse.rs` contextual-keyword retry loop — rewrites keyword-like text inside **string literals**, silently corrupting values.~~ **Resolved** — the retry renames the keyword at the parser's reported error offset instead of searching the source for the first `.keyword`. A keyword inside a literal is valid Python, so it is never the error location and can no longer be rewritten. See `specs/expr/parser.md` (Keyword rename fixup).
3. ~~`eval/evaluator.rs` `eval_number` float passthrough — literal source ranges computed against the parenthesized parse string are applied to the unwrapped source; multiline expressions mis-slice the display string.~~ **Resolved** — the paren-wrap shift is subtracted before slicing, and the slice must round-trip to the same `f64` or passthrough is declined, so an offset mismatch falls back to default formatting instead of displaying wrong text.
4. ~~`path_mapping.rs:150–155` (`split_path_parts`) — discards the absolute/relative distinction, so relative inputs match absolute rules.~~ **Resolved** — rootedness is compared explicitly before component matching, and the separator set now follows the rule's source format (Windows splits on `\` and `/`, Posix only on `/`, where a backslash is an ordinary filename character). Both halves match `PurePosixPath`/`PureWindowsPath` `is_relative_to`; see `specs/expr/path-mapping.md` (Application).
...
9. ~~`functions/list.rs:120` (`range_fn`) — unchecked `v += step` overflows near `i64::MAX`.~~ **Resolved** — `checked_add`; overflow ends the range, since the next value would necessarily be past `stop`.
10. ~~`functions/list.rs` (`sum_list`, RangeExpr branch) — unchecked `r.iter().sum::<i64>()`; the list branch uses `checked_add` correctly, the range branch does not (also iterates the range twice).~~ **Resolved** — the range branch accumulates with `checked_add` in a single pass and returns the standard integer-overflow error. (Location correction: `sum_list` lives in `functions/math.rs`, not `functions/list.rs`.)
...
12. ~~`functions/string.rs:212` (`split_fn`/`rsplit_fn`) — negative `maxsplit` wraps via `as usize`, then `n + 1` overflows; release builds produce `[]` where Python treats negative maxsplit as "no limit."~~ **Resolved** — negative maxsplit is rejected before conversion (treated as "no limit", matching `str.split`), the `i64 → usize` conversion saturates instead of casting so it stays lossless on 32-bit targets, and the `+ 1` is a `saturating_add`.
13. ~~`functions/regex.rs:363` (`re_split_fn`) — same `n + 1` overflow; also the only regex function using unchecked `make_list` instead of `make_list_checked`.~~ **Resolved** — `re_split` follows `re.split` semantics (negative → no splits, 0 → unlimited), uses the same saturating conversion as `split`/`rsplit`, and now builds its result with `make_list_checked`.
14. ~~`functions/string.rs` (`zfill_fn`) — negative width wraps to `usize::MAX` → `"0".repeat(huge)` allocation abort (inspection; not executed to avoid aborting the harness). Output size is also never charged to memory/op budgets.~~ **Resolved** — negative widths clamp to zero (string returned unchanged, matching Python) and the exact output byte count is preflighted against both the operation and memory budgets before allocation.
15. ~~`functions/string.rs` (`center_fn`/`ljust_fn`/`rjust_fn`) — negative width wraps huge; fails *closed* with a nonsensical "operation count (72057594037927938) exceeded limit" error where Python returns the string unchanged. The inconsistency with `zfill` (which fails *open*) shows the four width-taking functions lack a shared validation path.~~ **Resolved** — all four width-taking functions share a `padding_metrics` helper that clamps negative widths and preflights output bytes; see `specs/expr/function-library.md` (Preflighting Output Budgets).
16. ~~`functions/path.rs:526` (`path_starts_with`) — `path[..base.len()]` byte-slices at a possibly non-char boundary on Windows-format comparison; panics on multibyte UTF-8 paths. `relative_to_fn` shares the helper.~~ **Resolved** — the comparison is done on byte slices, which cannot panic at a mid-character index; ASCII case folding on bytes is equivalent, and a boundary that falls mid-character simply compares unequal. `relative_to_fn` inherits the fix through the shared helper.
...
**Spec compliance (Rust):** default limits match §1.3.9/§1.3.10 exactly; the full §2 function library is registered; `range_expr` slicing conforms to §2.1.8 (positive step → `range_expr`) **where the Python reference does not** (Python always materializes `list[int]`, unbounded and uncounted — worth reporting upstream). Confirmed Rust deviations:

1. **Over-broad `repr_*` registrations** — **Partially resolved:** `repr_sh` and `repr_cmd` now reject unsupported scalar and list types while preserving path coercion and empty-list behavior through exact internal overloads. Remaining example: `repr_pwsh(null)` still succeeds in Rust but errors in Python.
...
| X3 | ~~`range(9223372036854775806, 9223372036854775807, 2)` — **panic** `attempt to add with overflow`, `list.rs:120`~~ **Resolved** — the range terminates instead (regression tests `range_step_overflow_near_i64_max_terminates`, `range_step_underflow_near_i64_min_terminates`) | Panic |
| X4 | ~~`sum(range_expr('9223372036854775000-9223372036854775806:2'))` — **panic** `attempt to add with overflow` in `Sum` accumulation~~ **Resolved** — returns the integer-overflow error (regression test `range_expr_sum_overflow_is_error`); the accumulation lives in `functions/math.rs`, not `list.rs` | Panic |
| X5 | `round(1.5, -9223372036854775808)` | **panic** `attempt to negate with overflow`, `math.rs:170` | Panic |
| X6 | ~~`split('a b c', ' ', -1)` — **panic** `attempt to add with overflow`, `string.rs:212` (release: returns `[]`; Python returns `['a','b','c']`)~~ **Resolved** — returns `['a','b','c']` (regression tests `split_negative_maxsplit_means_no_limit`, `split_huge_maxsplit_means_no_limit`) | Panic / wrong value |
| X7 | ~~`re_split(' ', 'a b c', -1)` — **panic** `attempt to add with overflow`, `regex.rs:363`~~ **Resolved** — `re.split` semantics: negative means no splits (regression tests `re_split_negative_maxsplit_means_no_splits`, `re_split_huge_maxsplit_means_no_limit`) | Panic |
| X8 | ~~`path('日').is_relative_to('ab')` (Windows path format) — **panic** `byte index 2 is not a char boundary`, `path.rs:526`~~ **Resolved** — byte-slice comparison cannot panic mid-character (regression tests `is_relative_to_windows_multibyte_no_panic`, `is_relative_to_windows_multibyte_match`) | Panic on multibyte paths |
| X9 | ~~`'a.class' + X.class` (with `X.class` = `"c1"`) — returns `"a.alassc1"`; the **string literal** was rewritten by the contextual-keyword retry~~ **Resolved** — returns `"a.classc1"` (regression tests `keyword_retry_leaves_string_literals_alone`, `keyword_retry_leaves_literals_alone_in_either_order`) | Silent wrong value |
| X10 | ~~`12.5 if Cond else\n0.0` (Cond=true) — result displays as `"2.5 "` instead of `"12.5"`~~ **Resolved** — displays `"12.5"` (regression tests `multiline_float_passthrough_not_misaligned`, `multiline_float_passthrough_preserves_formatting`) | Silent wrong value |
| X11 | `ExprValue::Float(1e30).coerce(&INT, …)` | `Ok(9223372036854775807)` — silent saturation instead of error | Silent wrong value |
| X12 | ~~POSIX rule `/home/user → /mnt/x` applied to relative `"home/user/f"` — `Some("/mnt/x\f")`, a relative path matched an absolute rule~~ **Resolved** — returns `None` (regression test `relative_path_does_not_match_absolute_rule`). Found while fixing it: the same helper split on `\` under POSIX too, so `/home\user/f` also matched; also resolved (`posix_rule_does_not_match_backslash_as_separator`). The host-format separator in the output is a separate, intentional behaviour — `apply()` documents host-native joining for Python parity, and `apply_with_format()` exists for explicit control. | Wrong mapping |
| X13 | `9223372036854775807 == 9223372036854775807.0` | `true`; Python's exact comparison says `False` (float is 2^63) | Python divergence |
| X14 | ~~`center('a', -1)` — error "operation count (72057594037927938) exceeded limit (10000000)"; Python returns `'a'`~~ **Resolved** — negative widths clamp to zero; returns `'a'` (regression test `negative_padding_width_returns_input_unchanged`) | Wrong error/value |
| X15 | `repr_py('a\nb')` | `'a` + raw newline + `b'` — not a valid Python literal | Wrong repr |
| X16 | ~~`'日本語' + X.iffy` (byte/char index mismatch probe) — passed; not reproduced with this input (defect confirmed by inspection only)~~ **Resolved** — the boundary check that mixed a byte offset into a `chars()` index no longer exists; the keyword token is delimited entirely in byte space (regression test `keyword_retry_offsets_are_bytes_not_chars`) | — |

Not executed (would abort/hang the test harness; confirmed by inspection): ~~`zfill('5', -1)` → `"0".repeat(usize::MAX)` allocation abort.~~ **Resolved** — negative widths clamp to zero before any width arithmetic. ~~`[1,2,3,4] * 4611686018427387904` and `[] * 4611686018427387904` → uncounted loop of up to 2^62 iterations in release builds.~~ **Resolved** — these now return an Integer overflow error and `[]` respectively (regression tests `list_mul_length_overflow`, `empty_list_mul_huge_is_empty`).
...
1. **Adopt checked arithmetic at the function-library boundary.** Fix X1–X7 with `checked_add`/`checked_mul`/`checked_neg` (or saturating where semantically safe) in ~~`range_expr.rs:130` (+ `len()`/`contains()`)~~ (range_expr **Resolved** — endpoint/step magnitudes bounded below 2^62 at construction, keeping all derived arithmetic exactly in i64), ~~`arithmetic.rs` `mul_string`/`mul_list`~~ (**Resolved** — `checked_mul` + regression tests), ~~`list.rs` `range_fn`~~ (**Resolved** — `checked_add`, overflow ends the range) / ~~`sum_list`~~ (**Resolved** — single-pass `checked_add`; it is in `math.rs`), ~~`math.rs` `round_fn`~~ (**Resolved** — extreme-ndigits short-circuits, budgeted precision, regression tests), ~~`string.rs` `split_fn`/`rsplit_fn`~~ (**Resolved** — saturating `i64 → usize` conversion and `saturating_add`), ~~`regex.rs` `re_split_fn`~~ (**Resolved** — same saturating conversion). Add regression tests from the §7 table.
2. ~~**Validate negative int arguments before `as usize`.** `zfill`, `center`, `ljust`, `rjust`, `split`/`rsplit`/`re_split` maxsplit. Match Python semantics (negative maxsplit → no limit).~~ **Resolved** — `zfill`/`center`/`ljust`/`rjust` share a `padding_metrics` helper that clamps negative widths to Python's unchanged-string semantics and preflights output bytes against the op/memory budgets; `split`/`rsplit` treat negative maxsplit as "no limit" and `re_split` as "no splits", each matching its Python counterpart. All three maxsplit paths now convert `i64 → usize` with a saturating `try_from` rather than an `as` cast, so the conversion is lossless on 32-bit targets too.
3. ~~**Fix the contextual-keyword retry to skip string literals** (X9, `eval/parse.rs`) — corrupts user data silently. Also fix the byte-vs-char index mix in the after-keyword boundary check.~~ **Resolved** — the retry no longer searches the source at all; it renames at the offset the parser reported, which is never inside a literal. The boundary check went away with the search: the token is delimited by scanning to the first non-identifier character in byte space, so there is no `chars()`-index-versus-byte-offset mix left, and `.assert` is no longer readable as `.as` plus junk (`rename_keyword_at_*` unit tests).
4. ~~**Fix float-literal passthrough source slicing for multiline expressions** (X10, `eval/evaluator.rs` `eval_number`); have `Float64::with_str` validate that the string round-trips to the value as a backstop.~~ **Resolved** — the paren-wrap shift is subtracted before slicing, and the round-trip check is applied at the passthrough site (a slice that does not parse back to the same `f64` is declined, falling back to default formatting).
5. ~~**Fix `path_starts_with` char-boundary panic** (X8, `path.rs:526`) — use `get(..len)` or char-aware case-insensitive comparison.~~ **Resolved** — compares byte slices with `eq_ignore_ascii_case`, which is equivalent for ASCII case folding and cannot panic at a mid-character index.
6. ~~**Make `split_path_parts` preserve absoluteness** (X12, `path_mapping.rs:150`) so relative paths never match absolute rules; use the rule's source format (not host format) for output joining, or document why host format is intended.~~ **Resolved** — absoluteness is compared before component matching, and the separator set is taken from the rule's source format (this also fixed a POSIX over-match on `\`, found during the fix). Output joining keeps host-native separators deliberately: `apply()` mirrors Python's `os.name`-driven behaviour and `apply_with_format()` gives callers explicit control; both are documented in `specs/expr/path-mapping.md`.
...
10. **Trim `repr_*` registrations to the spec type sets** (§5.1) — **Partially resolved:** `repr_sh` and `repr_cmd` are constrained to their specified inputs (plus internal path/empty-list overloads needed for standard coercion semantics); the remaining representation functions still need review.
